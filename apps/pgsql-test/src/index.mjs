...
async function run() {
  ...
  const updatedConversation = await conversations.updateConversation(conversation.id, { title: '更新后的标题' })
  console.log('更新会话:', updatedConversation)

  console.log('\n=== 消息 CRUD ===')

  const userMessage = await messages.createMessage(conversation.id, 'user', '你好，请介绍一下 PostgreSQL')
  console.log('创建用户消息:', userMessage)

  const assistantMessage = await messages.createMessage(conversation.id, 'assistant', 'PostgreSQL 是一个功能强大的开源关系型数据库。')
  ...
  const updatedMessage = await messages.updateMessage(userMessage.id, '你好，请介绍一下 pgvector')
