...
export const QueryAugmentationSchema = z.object({
  queries: z.array(z.string()).length(3).describe('恰好 3 条中文检索问句：不同角度改写或扩写；保留订单号、品牌等字面信息；不要编造事实'),
})
...
function normalizeThreeQueries(original, list) {
  const out = (list ?? []).map((s) => (typeof s === 'string' ? s.trim() : '')).filter(Boolean)
...
export function retrievalQueryStrings(original, augmentation) {
  return [original, ...(augmentation?.queries ?? [])].map((s) => (typeof s === 'string' ? s.trim() : '')).filter(Boolean)
}
